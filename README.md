...
Browser → Firebase Hosting (static HTML from frontend/)
              ...
              ├── /vaults.html                      (requires Google sign-in + allowlist)
              ├── /vaults/{slug}                    (vault detail → albums list)
              ├── /vaults/{slug}/{albumSlug}         (album → photo grid + lightbox)
              ├── /vaults/{slug}/settings            (admin vault settings)
...
- **Hosting rewrite**: `/api/**` requests are proxied by Firebase Hosting to Cloud Run (same-origin, no CORS issues); `/vaults/{slug}` is rewritten to `vault-detail.html`; `/vaults/{slug}/{albumSlug}` to `vault-album.html`
...
│   ├── vaults.html          # Protected - Vaults listing (sign-in + allowlist)
...
> **Note:** Do not use `python -m http.server` — it cannot handle the `/vaults/{slug}` rewrites and will return 404 for vault detail pages.
...
Then open `http://localhost:5001/vaults.html` in a browser.
...
- [ ] Vault detail page (`/vaults/{slug}`) shows album grid
- [ ] Album page (`/vaults/{slug}/{albumSlug}`) shows photo grid with thumbnails
